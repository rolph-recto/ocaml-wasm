        ...
        let wasm_module =
          ...
          ++ Profile.(record ~accumulate:true generate)
              (fun { Lambda.code = lambda; required_globals } ->
                ...
                ++ Wasmgen.compile_implementation modulename)
        in
        let objfile = outputprefix ^ ".wast" in
        let oc = open_out_bin objfile in
        try
          Wasm_print.module_ oc 80 wasm_module;
          close_out oc;
          Stypes.dump (Some (outputprefix ^ ".annot"))
        with x ->
          close_out oc;
        ...
        let bytecode, required_globals =
          ...
          ++ Profile.(record ~accumulate:true generate)
              (fun { Lambda.code = lambda; required_globals } ->
                ...
                ++ fun bytecode -> begin
                    ignore (Wasmgen.compile_implementation modulename lambda);
